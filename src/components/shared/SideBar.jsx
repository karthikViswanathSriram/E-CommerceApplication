...
export const SideBar = ({isProfileLayout = false}) => {
  ...
  return (
    <div className='flex grow flex-col gap-y-7 overflow-y-auto 
    bg-custom-gradient px-6 pb-4'>
                        ...
                        {sideBarLayout.map((item)=>(
                            <li key={item.name}>
                                <Link
                                    to={item.href}
                                    className={
                                        `${pathName===item.href ? "bg-custom-blue text-white" 
